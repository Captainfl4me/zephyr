...
/* Private constants ---------------------------------------------------------*/
#define ST87EC_WRP_PRINTF_MAX_SIZE (1600U) /*!< in nb of char elements (aligned with ST87 max data packet size) */
#define ST87EC_WRP_BOOT_AT_CMD_MAX_SIZE (160U) /*!< in nb of char elements */
#define ST87EC_WRP_RX_FIFO_SIZE (2048U) /*!< in nb of char elements (beware: power of 2 value expected!) */
#define ST87EC_WRP_TX_FIFO_SIZE (2048U) /*!< in nb of char elements (beware: power of 2 value expected!) */
...
uint32_t ST87EC_Wrapper_SendByte(uint8_t* pByteBuffer, uint32_t Length)
{
    ...
    if ((Length > 0U) && (pByteBuffer != NULL)) {
        ...
        if (EcLibVars.BootMode != BOOT_UPLOAD_MODE) {
			...
			LOG_DBG("TX SPY: %.*s", Length, pByteBuffer);
        }
...
static void ST87EC_Wrapper_UartTxStart(void)
{
    ...
    if (ST87EC_WrapperVars.UartCtxt.Tx.TxState == ST87EC_WRP_DATA_TO_TX_NONE) {
		...
		ST87EC_Lib_Hal_EnableUartInterrupts(ST87EC_UART_FLAG_TX_COMPLETE);
    }
}
...
static inline ST87EC_Lib_Result_t ST87EC_Wrapper_RxDataParse(ST87EC_Wrapper_UartRxCtx_t* pUartRx)
{
    ...
    if (pUartRx->RspFound == ST87EC_WRP_NONE_FOUND) {
        ...
        if (((pUartRx->TagString[0U] == '+') && (pUartRx->TagString[1U] == 'C') && (pUartRx->TagString[2U] == 'M') && (pUartRx->TagString[3U] == 'E') && (pUartRx->DataMsgTagLen >= 4U))
            || ((pUartRx->TagString[0U] == '4') && (pUartRx->TagString[1U] == '0') && (pUartRx->DataMsgTagLen == 4U) && (EcLibVars.BootMode == BOOT_AT_MODE))) {
        ...
        || ((EcLibVars.BootMode == BOOT_AT_MODE) && (pUartRx->Fifo[pUartRx->FifoRdIdx] == '\r'))) /* String end in Boot mode */
    {
        ...
        if (pUartRx->FifoRdIdx0 > pUartRx->FifoRdIdx) {
			LOG_DBG("RX SPY: %.*s", ((uint32_t)(ST87EC_WRP_RX_FIFO_SIZE) + 1U - pUartRx->FifoRdIdx0), &pUartRx->Fifo[pUartRx->FifoRdIdx0]);
			LOG_DBG("RX SPY (split due to FIFO): %.*s", pUartRx->FifoRdIdx, &pUartRx->Fifo[0]);
        } else {
			LOG_DBG("RX SPY: %.*s", pUartRx->FifoRdIdx - pUartRx->FifoRdIdx0, &pUartRx->Fifo[pUartRx->FifoRdIdx0]);
        }
